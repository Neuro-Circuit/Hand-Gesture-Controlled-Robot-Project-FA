...
| **19** | `hands.pair.frame` | شکل هندسی بین دو دست | Two-Hand Frame Shape | pair | وقتی هر دو دست هم‌زمان دیده شوند، یک چهارضلعی بین نقاط مرجع دو دست (مچ و قاعده انگشت اشاره) ساخته می‌شود که با حرکت دست‌ها تغییر شکل/اندازه/زاویه می‌دهد - دقیقاً همان الگوی 'قاب‌گیری با دو دست' که در دموهای MediaPipe دیده می‌شود. | `corners`: چهار نقطه گوشه به ترتیب [پایین‌چپ, بالاچپ, بالاراست, پایین‌راست]، هرکدام [x, y], `width`: فاصله نرمال‌شده بین مچ دو دست (مثلاً برای کنترل زوم قابل استفاده است), `height`: میانگین فاصله مچ تا قاعده انگشت اشاره هر دست, `center`: [x, y] مرکز چهارضلعی, `angle_deg`: زاویه شیب خط اتصال دو دست بر حسب درجه, `area`: مساحت تقریبی چهارضلعی (نرمال‌شده، بین 0 و 1) |
